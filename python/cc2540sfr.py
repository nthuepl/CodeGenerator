'''
	This is an attempt to model CC2540 in terms of access to SFR.
	It is a candidate for restructuring.
	It can be separated into several parts:

	- definition
  	- the core 8051 SFRs, which are common across all 8051
  	- the MCU-specific SFRs
		(I suppose one way is to define subclasses? that add/subtract?)
		for instance, cc2540 does not have port P3 (so P3 needs to be
		"subtracted" from the definition), or it has a very
		different way to handle UART etc, but at least the PCON, PSW stuff is
		about the same.  but at the same time it can also have enhanced
		ways to handle each SFR, such as allowing xaddress alias.

	- operation: this should be largely reusable across different MCUs
		and maybe compilers as well? but to do this, we will need to
		define the register usage convention.

	Specifically, CC2540 defines XREG (2-byte address), which are
	independent of compiler.  Plus, all 8051s have SFR (1-byte address).
	Both look like SFRs to the user.
	...
	context of MCU, load it from cc2540mcu.py or whichever compiler.

	Usage: use the SFR class to construct SFR (or XREG).

	Status (1/8/2014): there is potential cleaning up. Maybe it should
	be subclassed from code capsule class.

	Status (9/2/2013): works with code capsule and inter-assignment with
	cc2540var.VAR class.  can code gen for byte and bit assignment,
	but slice assignment does not work yet.

	Status (8/26/2013): I entered the data for SFR and XREG (I think
	most of them.. but maybe I stopped right around USB stuff.
  what works now:
	>>> ADCCON1 = SFR('ADCCON1')  # normally constructor is called by MCU class
  Then you can do
	ADCCON1.STSEL   # which retrieves the slice <5:4> from ADCCON1

	You can also assign to it:
	>>> ADCCON1.STSEL = 0b10  # this writes binary 10 = decimal 2 to the slice
	As you type, you get the assembly code (sequence) that can be
	executed on the node side to acomplish the access. For now, we just
	assume data is passed in the accumulator A.
	Later we will do more (use memory location, stack, or registers).
	We just generate assembly for now but later can generate assembly
	code.

	What is needed are (1) on node side, need a dispatcher that can
	preserve registers and can receive data and jump to it to execute.
	To run from RAM, just set the MEMCTR.XMAP bit and jump or call
	it with upper address 0x8000  (mapping RAM into CODE space).
...
class SFR:
	...
	def __init__(self, sfrName, mcu=None):
		...
		'''
		import copy
		if CC2540SFR._SFR.has_key(sfrName):
			defn = copy.deepcopy(CC2540SFR._SFR[sfrName])
		elif CC2540SFR._XREG.has_key(sfrName):
			defn = copy.deepcopy(CC2540SFR._XREG[sfrName])
		else:
	...
	def writeSFR(self, value):
		...
		if isinstance(value, CodeCapsule):
			...
			self._code = CodeCapsule(self._mcu, dest=self,
					src=self)
		if (isinstance(value, SFR)) or (isinstance(value, cc2540var.VAR)) \
	...
	def _loadByte(self, address):
		'''This generates the MOV instruction to move in a byte if SFR,
		   or use MOVX if XREG (address > 255).
			 perhaps this method should be moved into codecapsule?
		'''
		if (self._address <= 0xff):
