'''
  This is the top-level cc2540 MCU class.
...
class CC2540MCU:
	def __init__(self, CG = None):
		...
		self.__dict__['_bufAddress'] = 0xEA2 ## was 0x0881 ## 
		self.__dict__['_reprMode'] = 'text'  # either text or binary
		self.__dict__['_appendReturnBuf'] = True
	
	def send(self, code, retval_type):
		reprMode = self.reprMode()
		if reprMode == 'exec':
			return self.__dict__['CG'].send(0, 0, code)
			#, retval_type=retval_type)
		if reprMode == 'flash':
			return self.__dict__['CG'].send(1, 4, code)
