...
class SFR:
	...
	def writeSFR(self, value):
		...
		if (isinstance(value, SFR)) or (isinstance(value, atmega2560var.VAR)) \
			...
			if (self._address > 0x5f):
				# R27:R26 are the DPTR. put I/O address in and store.
				self._emit('CLR R27')
				self._emit('LDI R26, 0x%x', self._address)
				self._emit('ST R24, X')
			else:
				# need to handle not only constant value but also SFR and VAR
				self._emit('OUT 0x%x, R24', self._address - 0x20)
...
#		return self._code
	...
	def _loadByte(self, address):
		...
		'''
		if (self._address <= 0x5f):
			self._emit('IN R%d, 0x%x', 24, address - 0x20)
		else:
			self._emit('CLR R27') # clear upper X
			self._emit('LDI R26, 0x%x', address) # X is like loading DPTR
			self._emit('LD R24, X') # this is like 8051's MOVX A, @DPTR
		# self._A = self._value 
		return self._code
