'''
	...
	Actually, there are SFR (1-byte address) and XREG (2-byte
	address). Both look like SFRs to the user.
	...
	context of MCU, load it from cc2540mcu.py.

	Usage: use the SFR class to construct SFR.
	I have not decoded whether SFR class should also be used for
	constructing XREG, which has not been implemented even though data
	is there.
...
11: Timer mode''',
							...
							'INT': {
								'slice': (0,0), 'default': 0, 'access': 'R/W',
...
class SFR:
	...
	def __setattr__(self, name, value):
		...
		if isinstance(value, codecapsule.CodeCapsule):
			self._code = value
		else:
			self._code = codecapsule.CodeCapsule(self._mcu, dest=self,
					src=self)
		...
		elif (isinstance(value, codecapsule.CodeCapsule)):
	...
	def writeSFR(self, value):
		...
		if (isinstance(value, SFR)) or (isinstance(value, cc2540var.VAR)) \
				or isinstance(value, codecapsule.CodeCapsule):
	...
	def __getattr__(self, name):
		...
		if len(name) == 2 and name[0] == '_' and name[1] in '01234567':
			self._code = codecapsule.CodeCapsule(self._mcu, src=self,
