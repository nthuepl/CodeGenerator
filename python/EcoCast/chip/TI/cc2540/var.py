'''
   This is an attempt to build up variables for CC2540.
	 I suppose a lot of it could be extracted out to a common 8051 set,
	 but let's stick to CC2540 because it has a few nice features such
	 as multiple DPTRs.
   ...
   we'll add array, struct, and pointers later.
	 ...
	 Usage: the user creates a variable on the CC2540 (MCU?),
...
class VAR:
	...
	def readVar(self, dest=None):
		...
		'''
		code = codecapsule.CodeCapsule(self._mcu, dest)
		self._code = code
		...
		if size == 1 and self._storageClass in ['bitMem']:
			...
			self._code = codecapsule.CodeCapsule(self._mcu, dest)
			...
			self._code = codecapsule.CodeCapsule(self._mcu, self._mcu.lookupVar('R3R2'))
	...
	def writeVar(self, value):
		...
		if self._storageClass in ['bitMem']:
			...
			elif isinstance(value, VAR) or isinstance(value, cc2540sfr.SFR) \
		...
		if isinstance(value, cc2540sfr.SFR) or isinstance(value, VAR) \
		  or isinstance(value, codecapsule.CodeCapsule):
			# assume 8 bit
			...
			if self._storageClass in ['iram']:
				...
				if isinstance(value, cc2540sfr.SFR):
...
if __name__ == '__main__':
	g = globals()
	for varName in CC2540VAR._VAR.keys():
		defn = CC2540VAR._VAR[varName]
