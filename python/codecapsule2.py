...
#
'''
	This file is currently specific to Eco (nRF24E1),
	but it is prime candidate
	for reorganization.  It should be broken into the 8051 part (which
	is reusable) and the code generator (which can be reusable, but the
	calling convention needs to be abstracted).

	Another thing is that both SFR and VAR shoud be subclasses of
	this CodeCapsule.
'''
import isa_8051
import nrf24e1var # import sizeInBits, isSigned
import copy

def sizeInBits(typeName):
	return nrf24e1var.sizeInBits(typeName)

def isSigned(typeName):
	return nrf24e1var.isSigned(typeName)
...
class CodeCapsule:
	...
	def emit(self, formatStr, *operands):
		...
		self._instructions.append(isa_8051.ISA8051(formatStr, *operands))
	...
	def __len__(self):
		...
		return len(self.getMachineCode())
	...
	def __repr__(self):
		...
		if (self.reprMode() == 'text'):	
			...
			return repr(reduce(lambda x,y: x+y, clone.getMachineCode()))
	...
	def appendReturnValue(self):
		...
		if (t == 'void') or (t is None):
			# just set length = 0
			self.emit('MOV 0x%x, #0x%x', self.getReturnBufferAddress(), 0)
		elif (sizeInBits(t) == 1):
			# put C into A
			self.emit('CLR A')
			self.emit('RLC A') # rotate the C bit left back into A.0
			# set length = 1
			self.emit('MOV 0x%x, #0x%x', self.getReturnBufferAddress(), 1)
		elif (sizeInBits(t) == 8):
			# set length = 1
			self.emit('MOV 0x%x, #0x%x', self.getReturnBufferAddress(), 1)
			# Modified 12/20/2013: use A for return value
			# (function calls return in R1, already copied to A)
			self.emit('MOV 0x%x, A', self.getReturnBufferAddress()+1)
		elif (sizeInBits(t) == 16):
			# set length = 2
			self.emit('MOV 0x%x, #0x%x', self.getReturnBufferAddress(), 2)
			# assuming use iram for buffer; move data from R2:R3 to buf[1:2]
			self.emit('MOV 0x%x, R%d', self.getReturnBufferAddress()+1, 2)
			self.emit('MOV 0x%x, R%d', self.getReturnBufferAddress()+2, 3)
		elif (sizeInBits(t) == 24):
			# set length = 3
			self.emit('MOV 0x%x, #0x%x', self.getReturnBufferAddress(), 3)
			# assuming use iram for buffer; move data from R1:R3 to buf[1:2]
			self.emit('MOV 0x%x, R%d', self.getReturnBufferAddress()+1, 1)
			self.emit('MOV 0x%x, R%d', self.getReturnBufferAddress()+2, 2)
			self.emit('MOV 0x%x, R%d', self.getReturnBufferAddress()+3, 3)
		elif (sizeInBits(t) == 32):
			# set length = 4
			self.emit('MOV 0x%x, #0x%x', self.getReturnBufferAddress(), 4)
			# assuming use iram for buffer; move data from R2:R5 to buf[1:4]
			self.emit('MOV 0x%x, R%d', self.getReturnBufferAddress()+1, 2)
			self.emit('MOV 0x%x, R%d', self.getReturnBufferAddress()+2, 3)
			self.emit('MOV 0x%x, R%d', self.getReturnBufferAddress()+3, 4)
			self.emit('MOV 0x%x, R%d', self.getReturnBufferAddress()+4, 5)
		else:
	...
	def __or__(self, other):
		...
		if sizeInBits(t) == 1:
			if isinstance(other, int) or isinstance(other, bool):
				...
				raise ValueError('bitwise-or type unsupported: %s' % other)
		elif sizeInBits(t) == 8:
			if isinstance(other, int):
				# should we check range?
	...
	def __neg__(self):
		...
		if sizeInBits(t) == 1:
			...
			self.emit('XCH A, R%d', 2)
			self.emit('CLR A')
			self.emit('RLC A') # get the carry bit into A, so it's either 0 or 1
			self.emit('XCH A, R%d', 3)  # save carry bit into R3, put old R3 int oA
			self.emit('CPL A')   # 1's complement for upper byte
			self.emit('ADD A, R%d', 3)  # add the carry 
	...
	def __eq__(self, other):
		...
		if sizeInBits(t) == 1:
			...
			if isinstance(other, int):
				...
				self.emit('CJNE A, #0x%x, 0x%x', other, 1)
				...
				self.appendCode(other)  # take code already generated,
				self.emit('CLR C') # assume not equal
				self.emit('CJNE A, 0x%x, 0x%x', temp.varAddress(), 1)
				self.emit('SETB C') # equal => set C to 1
				self.freeTemp(temp)
			else:
			...
			if (isinstance(other, int)):
				...
				self.emit('CLR C') # assume not equal
				self.emit('MOV A, R2')
				self.emit('CJNE A, #0x%x, 0x%x', other & 0xff, 5) # this is 3 bytes
				self.emit('MOV A, R3') # this is 1 byte
				self.emit('CJNE A, #0x%x, 0x%x', (other >> 8) & 0xff, 1) # 3 bytes
				self.emit('SETB C') # 1 byte
				self.emit('CPL C') 
			elif isinstance(other, codecapsule.CodeCapsule):
				...
				self.emit('POP ACC')
				# subtract lower byte.
				# add their difference (should be 0) to the higher byte, and compare.
				self.emit('CLR C')  # this clears C bit for subtract
				self.emit('SUBB A, R%d', 2)
				self.emit('ADDC A, R%d', 3)  # should not have carry; if so, make it not work
				self.emit('CLR C')  # after sticking C back into R1, clear it for ret val
				self.emit('POP ACC')  # grab saved higher-order byte
				self.emit('CJNE A, 0x%x, 0x%x', 3, 1) # compare higher bytes
				self.emit('SETB C')  # skip if false; set C = 1 if equal.
		else:
	...
	def __ne__(self, other):
		...
		if sizeInBits(t) == 1:
			...
			if (isinstance(other, int)):
				# compare with int literal
				self.emit('CLR C') # assume equal
				self.emit('MOV A, R2')
				self.emit('CJNE A, #0x%x, 0x%x', other & 0xff, 5)
				# equal so far, continue comparing
				self.emit('MOV A, R3')
				self.emit('CJNE A, #0x%x, 0x%x', (other >> 8) & 0xff, 1)
				self.emit('SETB C')
		else:
			# other types can be implemented later, but may
			# require converting to longer types.
