...
static __xdata uint8 accelEnabler = FALSE;
...
static __xdata uint8 timeEnabler = FALSE;

unsigned __xdata char newTime[TIME_LEN]={0};
unsigned __xdata char newDate[DATE_LEN]={0};
unsigned __xdata char newAlarm[ALARM_LEN]={0};
#endif
 ...
 * GLOBAL VARIABLES
 */
static __xdata unsigned char isFlashCode = FALSE;
static __xdata unsigned char codeSection = 0x00;
__xdata unsigned char Message[512];
__xdata unsigned char HeaderPacket[4];
__xdata unsigned char i;
__xdata attHandleValueNoti_t notify;
extern __xdata uint16 EcoExecProfileConnHandle;

__xdata unsigned char pg;
__xdata uint16 offset = 0x0000;
__xdata uint16 addr;
 ...
 * LOCAL VARIABLES
 */
static __xdata uint8 simpleBLEPeripheral_TaskID;   // Task ID for internal task/event processing

static __xdata gaprole_States_t gapProfileState = GAPROLE_INIT;

// GAP - SCAN RSP data (max size = 31 bytes)
static __xdata uint8 scanRspData[] =
{
  // complete name
...
static __xdata uint8 advertData[] =
{
...
// GAP GATT Attributes
static __xdata uint8 attDeviceName[GAP_DEVICE_NAME_LEN] = "Neo's BLE Node";

/*********************************************************************
 * LOCAL FUNCTIONS
 */
static void simpleBLEPeripheral_ProcessOSALMsg(__xdata osal_event_hdr_t *pMsg );
...
static __xdata char *bdAddr2Str (__xdata  uint8 *pAddr );
...
// GAP Role Callbacks
static __xdata gapRolesCBs_t simpleBLEPeripheral_PeripheralCBs =
{
...
static __xdata gapBondCBs_t simpleBLEPeripheral_BondMgrCBs =
{
...
static /*__xdata */simpleProfileCBs_t simpleBLEPeripheral_SimpleProfileCBs =
{
  simpleProfileChangeCB    // Charactersitic value change callback
};

// EcoExec GATT Profile Callbacks
static __xdata EcoExecProfileCBs_t EcoExecProfileCBs =
{
  EcoExecProfileChangeCB    // Charactersitic value change callback
};

// Accelerometer Profile Callbacks
static __xdata accelCBs_t accelerometerProfileCBs =
{
...
static __xdata timeserviceCBs_t timeserviceProfileCBs = 
{
...
{
	/* This is a "callback" by OSAL task scheduler, right? */
	i2s_call_u8();
  ...
  // Setup the GAP Peripheral Role Profile
  {
      ...
      static __xdata uint8 initial_advertising_enable = FALSE;
    #else
      // For other hardware platforms, device starts advertising upon initialization
      static __xdata uint8 initial_advertising_enable = TRUE;
    #endif

    // By setting this to zero, the device will go into the waiting state after
    // being discoverable for 30.72 second, and will not being advertising again
    // until the enabler is set back to TRUE
    static __xdata uint16 gapRole_AdvertOffTime = 0;

    static __xdata uint8 enable_update_request = DEFAULT_ENABLE_UPDATE_REQUEST;
    static __xdata uint16 desired_min_interval = DEFAULT_DESIRED_MIN_CONN_INTERVAL;
    static __xdata uint16 desired_max_interval = DEFAULT_DESIRED_MAX_CONN_INTERVAL;
    static __xdata uint16 desired_slave_latency = DEFAULT_DESIRED_SLAVE_LATENCY;
    static __xdata uint16 desired_conn_timeout = DEFAULT_DESIRED_CONN_TIMEOUT;
  ...
  {
    static __xdata uint32 passkey = 0; // passkey "000000"
    static __xdata uint8 pairMode = GAPBOND_PAIRING_MODE_WAIT_FOR_REQ;
    static __xdata uint8 mitm = TRUE;
    static __xdata uint8 ioCap = GAPBOND_IO_CAP_DISPLAY_ONLY;
    static __xdata uint8 bonding = TRUE;
...
//    } attHandleValueNoti_t;
  ...
  { 
#ifdef FLASH_DEBUG
    static __xdata unsigned char FlashCode[HAL_FLASH_WORD_SIZE*CODEGENERATOR_WRITING_FLASH_SIZE]={0};
#endif
...
static void peripheralStateNotificationCB( gaprole_States_t newState )
{
  switch ( newState )
  {
    case GAPROLE_STARTED:
      {
        static __xdata uint8 ownAddress[B_ADDR_LEN];
        static __xdata uint8 systemId[DEVINFO_SYSTEM_ID_LEN];
...
static void performPeriodicTask( void )
{
  static __xdata uint8 valueToCopy;
  static __xdata uint8 stat;
 ...
 * @return  none
 */
__xdata char *bdAddr2Str(__xdata  uint8 *pAddr )
{
  __xdata static uint8       i;
  __xdata static char        hex[] = "0123456789ABCDEF";
  __xdata static char str[B_ADDR_STR_LEN];
  __xdata static char        *pStr = str;
...
static void accelEnablerChangeCB( void )
{
  __xdata static bStatus_t status;
...
static void accelRead( void )
{
  ...
  static __xdata int16 x, y, z;
  static __xdata int16 new_x, new_y, new_z;
...
void timeserviceChangeCB( uint8 paramID )
{
  static __xdata bStatus_t status;
...
{
  MEMCTR |= 0x08;
  __asm__("LCALL Message+0x8000"); /* 0x8000 + Message */
  MEMCTR &= ~0x08;
}
