...
static uint8 accelXCoordinates[2] = { 0, 0 };
static uint8 accelYCoordinates[2] = { 0, 0 };
static uint8 accelZCoordinates[2] = { 0, 0 };
...
bStatus_t Accel_SetParameter( uint8 param, uint8 len, void *value )
{
  ...
  switch ( param )
  {
    ...
    case ACCEL_X_ATTR:
      if ( len == ACCEL_COORDINATES_LEN ) 
      {      
        VOID osal_memcpy( accelXCoordinates, value, ACCEL_COORDINATES_LEN );

        // See if Notification has been enabled
        GATTServApp_ProcessCharCfg( accelXConfigCoordinates, accelXCoordinates,
    ...
    case ACCEL_Y_ATTR:
      if ( len == ACCEL_COORDINATES_LEN ) 
      {      
        VOID osal_memcpy( accelYCoordinates, value, ACCEL_COORDINATES_LEN );

        // See if Notification has been enabled
        GATTServApp_ProcessCharCfg( accelYConfigCoordinates, accelYCoordinates,
                                    FALSE, accelAttrTbl, GATT_NUM_ATTRS( accelAttrTbl ),
                                    INVALID_TASK_ID );
      }
      else
    ...
    case ACCEL_Z_ATTR:
      if ( len == ACCEL_COORDINATES_LEN ) 
      {      
        VOID osal_memcpy( accelZCoordinates, value, ACCEL_COORDINATES_LEN );

        // See if Notification has been enabled
        GATTServApp_ProcessCharCfg( accelZConfigCoordinates, accelZCoordinates,
                                    FALSE, accelAttrTbl, GATT_NUM_ATTRS( accelAttrTbl ),
                                    INVALID_TASK_ID );
      }
      else
...
bStatus_t Accel_GetParameter( uint8 param, void *value )
{
  ...
  switch ( param )
  {
    ...
    case ACCEL_X_ATTR:
      VOID osal_memcpy( value, accelXCoordinates, ACCEL_COORDINATES_LEN );
      break;

    case ACCEL_Y_ATTR:
      VOID osal_memcpy( value, accelYCoordinates, ACCEL_COORDINATES_LEN );
      break;

    case ACCEL_Z_ATTR:
      VOID osal_memcpy( value, accelZCoordinates, ACCEL_COORDINATES_LEN );
      break;
